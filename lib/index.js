...
`;
	  ...
	  // Fix Author Array
	  if (content.author && _.isString(content.author)) {
				content.author = [content.author];
	  } else if (!content.author || !_.isArray(content.author)) {
				content.author = [];
	  }
	...
	generateTempFile() {
		const generateDefer = new Q.defer(),
			self = this;
...
`
	...
	makeCover() {
		...
		if (this.options.cover) {
			...
			if (this.options.cover.slice(0,4) === "http") {
				writeStream = request.get(this.options.cover).set({"User-Agent": userAgent});
	...
	downloadAllImage() {
		...
		if (!self.options.images.length) {
			...
			const deferArray = [];
			_.each(self.options.images, image => deferArray.push(self.downloadImage(image)));
			Q.all(deferArray)
	...
	genEpub() {
		...
		archive.on("end", function() {
			...
			rimraf(cwd, { disableGlob: true }, function(err) {
				if (err) {
					genDefer.reject(err);
				} else {
					genDefer.resolve();
