...
(function () {
  ...
  EPub = class EPub {
    ...
    mapContent(content, index) {
      var self;
      self = this;
      var fatherIndex = content.fatherIndex ? content.fatherIndex + '_' : '';
      console.log(fatherIndex)
      index = fatherIndex + index;
      // console.log('allowedAttributes f', JSON.stringify(allowedAttributes))
      var $, allowedXhtml11Tags, titleSlug;
      if (!content.filename) {
      ...
      $($("*").get().reverse()).each(function (elemIndex, elem) {
        ...
        for (k in attrs) {
          v = attrs[k];
          // console.log('attrs', k)
          if (indexOf.call(self.options.allowedAttributes, k) >= 0) {
            if (k === "type") {
              if (that.name !== "script") {
                $(that).removeAttr(k);
              }
      ...
      content.data = $.xml();
      if (content.childs) {
        content.childs = _.map(content.childs, this.mapContent.bind(this));
      }
      return content;
    ...
    eachContent(content) {
      ...
      if (content.childs) {
        _.each(content.childs, this.eachContent.bind(this));
      }
    ...
    generateTempFile() {
      ...
      _.each(this.options.content, this.eachContent.bind(this));
      ...
      Q.all([Q.nfcall(ejs.renderFile, opfPath, self.options), Q.nfcall(ejs.renderFile, ncxTocPath, self.options), Q.nfcall(ejs.renderFile, htmlTocPath, self.options)]).spread(function (data1, data2, data3) {
        ...
        console.error('arguments', err, arguments);
